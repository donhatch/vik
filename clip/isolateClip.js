...
if (process.argv.length < 3) {
  process.stderr.write("Usage: isolateClip.js inputFileName.stl -o [outputFormat:]outputFileName.amf (or whatever)\n");
  process.exit(1);
}
...
jscad.compile(compilable, {}).then(compiled => {
  ...
  if (true) {
    let thickness = 2.6; // exact wall thickness
    let clipThickness = 2.5;

    let bbox = csg.cube({
      corner1: [thickness,thickness,thickness],
      corner2: [30,30,thickness+clipThickness+.1],
    });

    process.stderr.write("Intersecting with bbox...\n");
    answer = answer.intersect(bbox);
